...
/*   Updated: 2025/03/16 08:18:36 by akyoshid         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*_get_sem_name(int init_sem_status)
{
	if (init_sem_status == INIT_SEM_PREPARE_FLAG)
		return ("prepare_flag");
	else if (init_sem_status == INIT_SEM_ERROR_FLAG)
		return ("error_flag");
...
void	_init_semaphore_error(t_sim_data *sim_data, int init_sem_status)
{
	...
	if (init_sem_status >= INIT_SEM_ERROR_FLAG)
