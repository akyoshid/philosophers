...
/*   Updated: 2025/02/22 00:56:10 by akyoshid         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	proc_args(int argc, char *argv[], t_sim_data *sim_data)
{
	...
	if (sim_data->philo_num <= 0 || sim_data->philo_num > PHILO_NUM_LIMIT)
		return (print_args_error(PHILO_NUM, sim_data->philo_num));
	sim_data->die_time = philo_atoi(argv[2]);
	if (sim_data->die_time <= 0)
		return (print_args_error(DIE_TIME, sim_data->die_time));
	sim_data->eat_time = philo_atoi(argv[3]);
	if (sim_data->eat_time <= 0)
		return (print_args_error(EAT_TIME, sim_data->eat_time));
	sim_data->sleep_time = philo_atoi(argv[4]);
	if (sim_data->sleep_time <= 0)
		return (print_args_error(SLEEP_TIME, sim_data->sleep_time));
	if (argc == 6)
	{
		sim_data->eat_limit = philo_atoi(argv[5]);
		if (sim_data->eat_limit <= 0)
			return (print_args_error(EAT_LIMIT, sim_data->eat_limit));
	}
	else
